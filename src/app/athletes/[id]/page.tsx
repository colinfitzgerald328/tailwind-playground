...
const AthleteDisplayPage = () => {
  ...
  return (
    ...
    {results.map((result, index) => (
      <div key={index} className="mb-4 pb-4 border-b last:border-b-0 flex items-start">
        <div className="mr-4">
          <Medal className={`w-6 h-6 ${getMedalColor(Number(  result.place))}`} />
        </div>
        <div>
          <p className="font-semibold text-lg">{result.competition}</p>
          <p className="text-gray-600">{new Date(result.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
      ...
      <div className="space-y-6">
        {top_competitors.map((competitor) => (
          <div key={competitor.athlete_id} className="border rounded-lg p-4 hover:shadow-md transition duration-300">
            <div className="flex flex-col md:flex-row md:items-center md:space-x-4">
            <Link href={`/athletes/${competitor.athlete_id}`} passHref>
              <img 
                src={competitor.hq_images?.[0] || "/api/placeholder/400/300"} 
              ...
              <div className="flex-grow">
                ...
                <p className="text-gray-600">{competitor.primary_disciplines}</p>
                <button 
                  onClick={() => setExpandedAthlete(expandedAthlete === competitor.athlete_id ? null : competitor.athlete_id)}
                  className="mt-2 text-blue-600 hover:text-blue-800"
                >
                  {expandedAthlete === competitor.athlete_id ? 'Show less' : 'Show more'}
                </button>
              </div>
            </div>
            {expandedAthlete === competitor.athlete_id && (
              <div className="mt-4">
                {competitor.markdown_summary && (
                  <div className="mb-4">
                    <ReactMarkdown>{competitor.markdown_summary}</ReactMarkdown>
                  </div>
                )}
                {competitor.hq_images && competitor.hq_images.length > 1 && (
